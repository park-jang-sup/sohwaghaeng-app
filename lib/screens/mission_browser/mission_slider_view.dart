...
class MissionSliderView extends StatefulWidget {
  ...
  const MissionSliderView({
    super.key,
    required this.pageController, // ✅ 필수 인자
...
class _MissionSliderViewState extends State<MissionSliderView> {
  // ❌ 삭제: late PageController _pageController;
  int _currentSlideIndex = 0;

  @override
  void initState() {
    super.initState();
    // ❌ 삭제: _pageController = PageController...

    // ✅ [수정 2] 화면이 다시 그려질 때 현재 페이지 위치를 컨트롤러에서 가져옴
  ...
  @override
  void dispose() {
    // ❌ 삭제: _pageController.dispose(); (부모가 해줌)
    super.dispose();
  }

  // ✅ [수정 3] 자석 효과 함수 (widget.pageController 사용)
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                // ✅ [수정 4] Listener + Stack이 적용된 슬라이더 빌더 사용
                _buildSlider(),
  ...
  Widget _buildSlider() {
    ...
    return SizedBox(
      ...
      child: Stack(
        children: [
          // ✅ [수정 5] Listener를 사용하여 드래그 끊김 현상 방지 코드 부활
          Listener(
            onPointerMove: (details) {
              if (widget.pageController.hasClients) {
                widget.pageController.jumpTo(
                    widget.pageController.offset - details.delta.dx);
              }
            },
            onPointerUp: (_) => _snapToNearestPage(),
            onPointerCancel: (_) => _snapToNearestPage(),
            child: PageView.builder(
              controller: widget.pageController, // ✅ 부모 컨트롤러 연결
              itemCount: widget.filteredMissions.length,
              physics: const NeverScrollableScrollPhysics(), // 직접 제어하므로 스크롤 끔
          ...
          Positioned(
            ...
            child: Center(
              child: SliderNavButton(
                ...
                onTap: () => widget.pageController.previousPage( // ✅ 수정
                  duration: const Duration(milliseconds: 300),
          ...
          Positioned(
            ...
            child: Center(
              child: SliderNavButton(
                ...
                onTap: () => widget.pageController.nextPage( // ✅ 수정
                  duration: const Duration(milliseconds: 300),
                  curve: Curves.easeInOut,
                ),
              ),
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: '소확행을 검색하세요', prefixIcon: Icon(Icons.search, color: Colors.grey), border: InputBorder.none,
                    contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  ),
                ),
              ),
            ),
            IconButton(icon: Icon(Icons.refresh, color: Colors.grey.shade600), onPressed: widget.onRefresh),
            IconButton(icon: Icon(Icons.filter_list, color: widget.selectedFilterTag != null ? Colors.orange : Colors.grey.shade600), onPressed: widget.onFilterTap),
          ],
        ),
      ),
    );
  ...
  Widget _buildFeaturedSection() {
    ...
    final topMissions = List<BrowserMission>.from(widget.allMissions)..sort((a, b) => b.likes.compareTo(a.likes));
      ...
      child: Column(
        ...
        children: [
          ...
          ...featured.map((m) => FeaturedMissionCard(
              mission: m, isLiked: widget.likedMissions.contains(m.id), onLikeTap: () => widget.onToggleLike(m.id)
          )),
        ],
      ),
    );
  ...
  Widget _buildListHeader() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Row(children: [
            ViewModeButton(icon: Icons.grid_view_rounded, mode: 'grid', currentMode: widget.viewMode, onModeChanged: widget.onViewModeChanged),
            const SizedBox(width: 8),
            ViewModeButton(icon: Icons.view_carousel_rounded, mode: 'slider', currentMode: widget.viewMode, onModeChanged: widget.onViewModeChanged),
          ]),
          TextButton(
            onPressed: () => widget.onSortChanged(widget.sortBy == 'latest' ? 'likes' : 'latest'),
            child: Row(children: [Icon(widget.sortBy == 'latest' ? Icons.access_time : Icons.favorite, size: 16, color: Colors.grey), const SizedBox(width: 4), Text(widget.sortBy == 'latest' ? '최신순' : '좋아요순', style: const TextStyle(color: Colors.grey))]),
          )
        ],
      ),
    );
